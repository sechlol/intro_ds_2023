...
def run_pipeline(data: pd.DataFrame, y_target: np.ndarray) -> Sequential:
    ...
    model.fit(x_train, y_train, batch_size=_BATCHES, epochs=_EPOCHS, validation_split=0.15, shuffle=True)
    return model
...
def save_result(result: LstmResultData, split: Tuple):
